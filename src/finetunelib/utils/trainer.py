...
from finetunelib.monkeypatch.relora import ReLoRACallback, ReLoRAScheduler
from finetunelib.utils.callbacks import (
    GPUStatsCallback,
    SaveBetterTransformerModelCallback,
    SavePeftModelCallback,
    bench_eval_callback_factory,
)
from finetunelib.utils.collators import DataCollatorForSeq2Seq
from finetunelib.utils.dataloader import MultipackDistributedDataloader
from finetunelib.utils.distributed import is_main_process, zero_first
from finetunelib.utils.schedulers import get_cosine_schedule_with_quadratic_warmup

LOG = logging.getLogger("finetunelib")
...
@dataclass
class FineTuneLibTrainingArguments(TrainingArguments):
    """
    Extend the base TrainingArguments for finetunelib helpers
    """
...
class FineTunelibTrainer(Trainer):
    """
    Extend the base Trainer for finetunelib helpers
    """

    args = None  # type: FineTunelibTrainingArguments
...
class OneCycleLRSchedulerTrainer(FineTunelibTrainer):
    """
...
class ReLoRATrainer(FineTunelibTrainer):
    """
    Trainer subclass that uses the OneCycleLR scheduler
    """
...
def setup_trainer(cfg, train_dataset, eval_dataset, model, tokenizer, total_num_steps):
    ...
    training_args = FineTunelibTrainingArguments(  # pylint: disable=unexpected-keyword-arg
    ...
    if cfg.is_llama_derived_model and cfg.landmark_attention:
        from finetunelib.monkeypatch.llama_landmark_attn import (
            add_mem_tokens,
            get_mem_id,
            set_model_mem_id,
        )
    ...
    trainer_cls = FineTunelibTrainer
